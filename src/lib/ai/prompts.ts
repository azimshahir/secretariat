...
10. Treat each entry's sampleValue or sampleItems as a structural example of what kind of content belongs there. Do NOT copy previous-minute wording verbatim unless the current agenda evidence genuinely supports the same sentence. EXCEPTION: entries with sourceConstraint = paper — for those, the sampleValue IS the required sentence scaffold (see rule 21).
...
16. Use the CANONICAL FIVE-PART REPORT as the primary synthesized content source. Use the transcript, fact-grounding analysis, agenda PDF facts, and RELEVANT MIND INSTRUCTIONS to write fresh agenda-specific content. Template sample prose is not evidence and must not be pasted as final content. EXCEPTION: entries with sourceConstraint = paper — for those, follow rule 21 instead; the sample IS the sentence scaffold and the AGENDA PAPER ORIGINAL TEXT is the evidence source.
...
21. CRITICAL — If an entry has sourceConstraint = paper, this rule overrides rules 10 and 16 for that entry. The entry's sampleValue is the REQUIRED sentence scaffold — keep its exact sentence structure, wording, and paragraph flow. Find the matching current-period data in the AGENDA PAPER ORIGINAL TEXT section and substitute ONLY the dates, figures, percentages, ratings, scores, and named values with the current values from the paper. Do NOT rewrite the sentences. Do NOT use transcript content. Do NOT generate fresh prose. The output must read like the sampleValue with updated numbers from the paper. If the paper has no matching data for a specific figure, keep the scaffold sentence and wrap the missing value in [[VERIFY: original value]].
